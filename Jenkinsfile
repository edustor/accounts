node {
    ...
    stage("Prepare")
    baseImage = docker.build("edustor/accounts-base", "-f Dockerfile.base .")
    ...
    buildImage = baseImage.inside("-v $dir:/code -v /mnt/media/jenkins/cache/.gradle:/root/.gradle") {
        ...
        stage "Assemble"
        sh "./gradlew assemble"
        sh "mv build/dist/edustor-accounts.jar ."
        archiveArtifacts "edustor-accounts.jar"
        fingerprint "edustor-accounts.jar"
    }

    if (env.BRANCH_NAME == "master") {

        stage("Build image") {
            image = docker.build("edustor/accounts:ci-$env.BUILD_NUMBER")
        }
